...
<div class="fixed inset-0 z-40 pointer-events-none flex items-center justify-center" style="visibility: {isOpen ? 'visible' : 'hidden'}">
  ...
  <div class="card-base max-w-5xl h-[85vh] shadow-2xl flex flex-col pointer-events-auto transition-all duration-500 ease-out {isOpen ? 'translate-y-0 opacity-100 scale-100' : 'translate-y-8 opacity-0 scale-95'} overflow-hidden relative">
    
    <div class="px-8 pt-6 bg-rx-cream/50 border-b border-rx-peach flex flex-col gap-6 shrink-0">
      <div class="flex justify-between items-center">
        <div>
          <h2 class="text-heading text-2xl">System Logs & Analytics</h2>
          <p class="text-subheading text-sm mt-1">Sanatorium Command Center</p>
        </div>
      ...
      <div class="flex gap-8 relative">
        <button onclick={() => activeTab = 'feed'} class="pb-3 text-sm font-bold tracking-wide transition-colors {activeTab === 'feed' ? 'text-rx-green' : 'text-rx-green/50 hover:text-rx-green'}">Activity Feed</button>
        <button onclick={() => activeTab = 'defaulters'} class="pb-3 text-sm font-bold tracking-wide transition-colors {activeTab === 'defaulters' ? 'text-red-600' : 'text-rx-green/50 hover:text-rx-green'} flex items-center gap-2">
          Action Required
        ...
        <button onclick={() => activeTab = 'analytics'} class="pb-3 text-sm font-bold tracking-wide transition-colors {activeTab === 'analytics' ? 'text-rx-green' : 'text-rx-green/50 hover:text-rx-green'}">Inventory & Morbidity</button>
        
        <div class="absolute bottom-0 h-0.5 bg-rx-green transition-all duration-300 rounded-t-md" 
    ...
    <div class="flex-1 overflow-y-auto p-8 bg-rx-cream">
      
      {#if isLoading}
        <div class="h-full w-full flex flex-col items-center justify-center gap-4 text-rx-green/50">
          <svg class="w-8 h-8 animate-spin" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
          <p class="tag-badge text-rx-green/50">Querying Database...</p>
        </div>

      {:else if activeTab === 'feed'}
        <div class="max-w-3xl mx-auto flex flex-col gap-4">
          {#if activityLogs.length === 0}
            <p class="text-center text-rx-green/50 font-medium py-10">No recent activity found.</p>
          {/if}
          
          {#each activityLogs as log}
            <div class="bg-white border border-rx-peach rounded-xl p-5 shadow-sm flex items-start gap-4 hover:border-rx-green/50 transition-colors">
              <div class="mt-1 w-8 h-8 rounded-full bg-rx-cream flex items-center justify-center border border-rx-peach shrink-0">
                <svg class="w-4 h-4 text-rx-green" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>
              </div>
              <div class="flex-1">
                <div class="flex justify-between items-start">
                  <p class="text-sm font-semibold text-rx-green">
                    <span class="bg-rx-peach/50 px-2 py-0.5 rounded text-xs text-rx-green font-bold mr-2 uppercase tracking-wider">{log.action || 'Dispensed'}</span>
                    {log.drug_name}
                  </p>
                  <span class="text-xs font-medium text-rx-green/50 shrink-0">{formatTime(log.created_at)}</span>
                </div>
                {#if log.students}
                  <p class="text-sm text-rx-green/70 mt-1.5 font-medium">To: <span class="font-bold text-rx-green">{log.students.first_name} {log.students.last_name}</span> ({log.students.reg_no})</p>
                {/if}
                {#if log.notes}
                  <p class="text-xs text-rx-green/70 mt-2 bg-white/50 p-2 rounded-md border border-rx-peach">{log.notes}</p>
                {/if}
              </div>
            </div>
          {/each}
      ...
      {:else if activeTab === 'analytics'}
        <div class="grid grid-cols-2 gap-8 max-w-5xl mx-auto">
           ...
           <div class="card-base border-rx-peach overflow-hidden flex flex-col !p-0">
              <div class="p-6 border-b border-rx-peach bg-rx-cream/50 flex items-center gap-3">
                 <div class="w-8 h-8 rounded-lg bg-blue-100 text-blue-600 flex items-center justify-center">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"></path></svg>
                 </div>
                 <h3 class="text-heading text-lg">Inventory Burn Rate</h3>
              </div>
              <div class="p-6 flex-1 overflow-y-auto">
                 <p class="tag-badge text-rx-green/50 mb-4 !px-0">Dispensing Volume (Recent)</p>
                 <div class="flex flex-col gap-3">
                    {#each Object.entries(inventoryBurn) as [drug, count]}
                       <div class="flex items-center justify-between">
                          <span class="text-sm font-semibold text-rx-green truncate pr-4">{drug}</span>
                          <span class="text-sm font-bold text-rx-green bg-rx-peach/50 px-3 py-1 rounded-full">{count} units</span>
                       </div>
                       <div class="w-full h-1.5 bg-rx-peach/30 rounded-full overflow-hidden mb-2">
                          <div class="h-full bg-rx-green rounded-full" style="width: {Math.min((count / 50) * 100, 100)}%;"></div>
                       </div>
                    {/each}
                    {#if Object.keys(inventoryBurn).length === 0}
                       <p class="text-sm text-rx-green/50 font-medium">Not enough data to calculate burn rate.</p>
                    {/if}
                 </div>
              </div>
           </div>
           ...
           <div class="card-base border-rx-peach overflow-hidden flex flex-col !p-0">
              <div class="p-6 border-b border-rx-peach bg-rx-cream/50 flex items-center gap-3">
                 ...
                 <h3 class="text-heading text-lg">Morbidity Trends</h3>
              </div>
              <div class="p-6 flex-1 overflow-y-auto">
                 <p class="tag-badge text-rx-green/50 mb-4 !px-0">Symptom Frequency</p>
                 <div class="flex flex-wrap gap-2">
                    {#each Object.entries(morbidityTrends) as [symptom, count]}
                       <div class="px-3 py-2 card-base !shadow-sm !p-2 border border-rx-peach flex items-center gap-3">
                          <span class="text-sm font-bold text-rx-green">{symptom}</span>
                    ...
                    {#if Object.keys(morbidityTrends).length === 0}
                       <p class="text-sm text-rx-green/50 font-medium">Not enough symptom data logged yet.</p>
                    {/if}
                 </div>
              </div>
           </div>
